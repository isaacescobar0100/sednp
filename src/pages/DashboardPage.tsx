    ...
    <div className="mx-auto max-w-[1440px]">
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-3">
        <section className="rounded-2xl border border-ink/[0.08] bg-white p-5 xl:col-span-2">
          <div className="mb-5"><h2 className="font-display text-base font-semibold">Recaudo mensual</h2><p className="mt-1 text-xs text-ink/50">Ingresos confirmados por mes · COP</p></div>
          {revenue.length > 0 ? (
            <div className="h-64"><ResponsiveContainer width="100%" height="100%"><AreaChart data={revenue} margin={{ top: 8, right: 4, bottom: 0, left: 4 }}><defs><linearGradient id="revenueArea" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#C9973B" stopOpacity={0.34} /><stop offset="100%" stopColor="#C9973B" stopOpacity={0} /></linearGradient></defs><XAxis dataKey="month" tick={{ fontSize: 11, fill: '#768094' }} axisLine={false} tickLine={false} /><YAxis width={64} tickFormatter={(v) => formatCopShort(Number(v))} tick={{ fontSize: 11, fill: '#768094' }} axisLine={false} tickLine={false} /><Tooltip formatter={(value: number) => [formatCopShort(Number(value)), 'Recaudo']} contentStyle={{ borderRadius: 12, border: '1px solid #e8e7e2', fontSize: 12 }} /><Area type="monotone" dataKey="income" stroke="#C9973B" strokeWidth={2.5} fill="url(#revenueArea)" /></AreaChart></ResponsiveContainer></div>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-3">
        <section className="rounded-2xl border border-ink/[0.08] bg-white p-5 xl:col-span-2"><div className="mb-5"><h2 className="font-display text-base font-semibold">Egresos por categoría</h2><p className="mt-1 text-xs text-ink/50">Gastos comprometidos (aprobados y pagados) · COP</p></div>
          {hasCategory ? (
            <div className="h-56"><ResponsiveContainer width="100%" height="100%"><BarChart data={byCategory} margin={{ top: 0, right: 8, bottom: 0, left: 4 }}><XAxis dataKey="category" tick={{ fontSize: 11, fill: '#768094' }} axisLine={false} tickLine={false} /><YAxis width={64} tickFormatter={(v) => formatCopShort(Number(v))} tick={{ fontSize: 11, fill: '#768094' }} axisLine={false} tickLine={false} /><Tooltip formatter={(value: number) => [formatCopShort(Number(value)), 'Egresos']} cursor={{ fill: '#F7F6F2' }} contentStyle={{ borderRadius: 12, border: '1px solid #e8e7e2', fontSize: 12 }} /><Bar dataKey="value" fill="#0F1B3D" radius={[6, 6, 0, 0]} barSize={30} /></BarChart></ResponsiveContainer></div>
